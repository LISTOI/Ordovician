...
module fma_pipeline #(
    ...
    reg [FPfrac : 0]        fracA_s1, fracB_s1, fracC_s1;
    ...
    always @(posedge clk) begin
        if(!rst_n) begin
            valid_s1 <= 1'b0;
            signA_s1 <= 0; expA_s1 <= 0; fracA_s1 <= 0;
            signB_s1 <= 0; expB_s1 <= 0; fracB_s1 <= 0;
            signC_s1 <= 0; expC_s1 <= 0; fracC_s1 <= 0;
        end else begin
            valid_s1 <= valid_in;
            signA_s1 <= signA_in;
            expA_s1 <= expA_in;
    ...
    reg valid_s2;
    reg signAB_s2;
    reg [FPexp + 1 : 0] expAB_s2;
    reg [((FPfrac + 1) << 1) - 1 : 0] prodAB_s2;
    reg signC_s2;
    ...
    always @(posedge clk) begin
        if(!rst_n) begin
            valid_s2 <= 0;
            signAB_s2 <= 0; expAB_s2 <= 0; prodAB_s2 <= 0;
            signC_s2 <= 0; expC_s2 <= 0; fracC_s2 <= 0;
        end else begin
            valid_s2 <= valid_s1;
    ...
    // Stage 5: Final normalize and pack
    reg valid_s5;
    reg [FP - 1 : 0] out_s5;

    integer shift_amt;
    reg [FPfrac : 0] norm_frac;
    reg [FPexp - 1 : 0] norm_exp;

    always @(*) begin
        norm_frac = frac_s4;
        norm_exp = exp_s4;
        for (shift_amt = FPfrac; shift_amt > 0 && norm_frac[FPfrac] == 0; shift_amt = shift_amt - 1) begin
            norm_frac = norm_frac << 1;
            if (norm_exp > 0) norm_exp = norm_exp - 1;
        end
    end
